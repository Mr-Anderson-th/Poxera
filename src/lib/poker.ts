// Port ตรงจากเว็บหลัก: ppch-poker-tracking/src/lib/points.ts
// ห้ามแก้ logic/ชื่อ function — ให้ผลลัพธ์ตรงกับเว็บ 100%
...
const WSOP_BASE: Array<[number, number, number]> = [
  [25, 50, 0], [50, 100, 0], [75, 150, 25], [100, 200, 25], [150, 300, 50],
  [200, 400, 50], [300, 600, 75], [400, 800, 100], [500, 1000, 100], [700, 1400, 200],
  [1000, 2000, 300], [1500, 3000, 400], [2000, 4000, 500], [3000, 6000, 1000], [4000, 8000, 1000],
  [5000, 10000, 2000], [7000, 14000, 2000], [10000, 20000, 3000], [15000, 30000, 5000], [20000, 40000, 5000],
  [30000, 60000, 10000], [50000, 100000, 15000], [75000, 150000, 25000], [100000, 200000, 30000], [150000, 300000, 50000],
];
...
export function buildBlindLevels(
  ...
  for (let i = 0; i < count; i++) {
    out.push({ level: i + 1, sb: roundChip(sb), bb: roundChip(bb), ante: i >= 3 ? roundChip(sb / 2) : 0 });
    sb = sb * m;
    bb = bb * m;
  }
  return out;
...
export function computePlayerAxes(
  ...
  const cashRate = mine.length ? (mine.filter((r) => Number(r.payout) > 0).length / mine.length) * 10 : 0;
